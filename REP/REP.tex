...
\newgeometry{hmargin={3cm,2.5cm},top=2.5cm,bottom=2.5cm, footskip=1.2cm} %modifica come più ti aggrada
...
Electricity bills are among the most important expenses for families around the world, at least for the lucky households that have access to it. According to the \textit{Energy Information Administration}, ``Electricity prices generally reflect the cost to build, finance, maintain, and operate power plants and the electricity grid."~\cite{weron2014electricity} The peculiarity of this good is in the fact it is produced in the moment it is consumed. In fact, electricity storage is still quite expensive, so providers usually decide to sell at a low price rather than store.~\cite{fuchs2012technology} Thus, short-term prices are impacted by the timing\todo{time} in consideration, day or night, morning or afternoon, but also by climate conditions: demand due to heating in the winter and cooling in the summer are the main drivers for seasonal price spikes. This is even truer\todo{more relevant} when not only the demand side but also the supply\todo{supply one} is influenced by the weather. One of the obstacles in the broad spread of renewable sources is precisely its reliance on external climate\todo{qui credo che climate sia sbagliato (non è sinonimo di weather), non solo, la marea non è climatica; toglierei l'aggettivo} conditions, e.g. sun, wind or tide.

The idea of this paper\todo{tecnicamente non è un paper. Thesis? Study?} is to propose state-of-the-art\todo{mi sembra un po' troppo, alla fine i modelli non sono proprio ultimo grido (non che sia un male)} machine learning approaches to \textit{price forecasting} in the electricity market domain\todo{toglierei domain}. The aim however is not to fit the monthly or even daily fluctuations in the price, but the longer-term swings, typically taking place at year level. For this reason, the focus is going to be on the demand side rather than on the supply, and indicators used have to do with the economic situation, the electricity market structure and the sources for energy generation.
...
Econometric studies on electricity price fluctuations are traditionally not only limited in the number of factors being studied, but also in the number of countries considered\todo{Girerei la frase: Traditionally, in econometric..., the number of factors... and the number of... is quite limited }. This is widely justifiable: the narrower the scope of the problem, the greater the confidence in the final result. This study\todo{however, on the other hand} starts from different assumptions, with the purpose shifting to the identification of those economic, market and energy choice indicators that allow to build valid\todo{manca qualcosa...} in Europe and worldwide. The models proposed are likely\todo{unlikely to be optimal} not to be optimal, i.e.~the overall best\todo{non mi pare spieghi molto... è ottimale nel senso che è il migliore, se vuoi spiegare di' che intendi in termini di predizione (o altro)}, for specific countries or even small geographical areas, yet understanding the exact nature of those local dependencies is not really the aim of this paper\todo{see above}. The geographical scope, in fact, is on\todo{credo non ci voglia on} the whole European continent, although findings\todo{the results} are also benchmarked using data from an even\todo{a broader} broader set of countries, all over the world.

The analysis is going to start from\todo{starts with} a presentation of the factors at stake for long-term price prediction. With this aim, \textit{chapter 2}\todo{Chapter (ma è gusto personale) + usa le etichette! label ref} summarizes country-wide differences in consumption patterns, energy generation choices and economic development. Once it is clear why differences in those indicators are expected to explain gaps in the electricity price between countries, in \textit{chapter 3} statistical models are proposed in order to test those expectations. Finally, the conclusions\todo{In Chapter 4} provide a comparison of performances among models, also exposing the areas this analysis may be improved.

\chapter{Elements at Stake}

\textit{Figure~\ref{fig:world_cost}} provides a global picture of the amount spent by households around the world for their electricity bills. Countries are colored on the basis of its price, measured in US dollars per kWh\todo{ho allargato un po' la pagina in modo che ci stesse il codice e si possano ingrandire un po' le immagini, modifica (sopra nel preambolo e più sotto per le varie immagini) come meglio credi}.
