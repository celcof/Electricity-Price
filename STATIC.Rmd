...
Getting data from world bank. Electricity Production From Coal Sources (% Of Total).

```{r}
X$geo <- as.factor(as.character(X$geo))
I <- wb(indicator = "EG.ELC.COAL.ZS", startdate = 1990, enddate = 2020, country=c(levels(X$geo), "GB", "GR"))
I$iso2c <- gsub("GB", "UK", I$iso2c)
I$iso2c <- gsub("GR", "EL", I$iso2c)
I <- I[,c(2,3,6)]
names(I) <- c("time", "COA", "geo")
X <- merge(X, I, by=c("time", "geo"), all.x=TRUE)
```
...
```{r}
I <- wb(indicator = "EG.ELC.NUCL.ZS", startdate = 1990, enddate = 2020, country=c(levels(X$geo), "GB", "GR"))
I$iso2c <- gsub("GB", "UK", I$iso2c)
I$iso2c <- gsub("GR", "EL", I$iso2c)
I <- I[,c(2,3,6)]
names(I) <- c("time", "NUC", "geo")
X <- merge(X, I, by=c("time", "geo"), all.x=TRUE)
```
...
```{r}
I <- wb(indicator = "EG.ELC.HYRO.ZS", startdate = 1990, enddate = 2020, country=c(levels(X$geo), "GB", "GR"))
I$iso2c <- gsub("GB", "UK", I$iso2c)
I$iso2c <- gsub("GR", "EL", I$iso2c)
I <- I[,c(2,3,6)]
names(I) <- c("time", "HYD", "geo")
X <- merge(X, I, by=c("time", "geo"), all.x=TRUE)
```
