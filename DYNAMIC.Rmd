...
Also add column for previous year electricity price.

```{r}
C$pPRI <- X[X$time != 2018, "PRI"]
```

In this case we also decide to add inflation column.
...
We split in training and validation sets.
...
So we decide to a specular logarithm transformation.
...
```{r}
LM1 <- lm(PRI ~ DEP + CON + GDP + OIL + REN + NUC + EMI + MAR + CRI + INF, C, subset=train)
summary(LM1)
```
