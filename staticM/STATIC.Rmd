...
```{r}
LMc <- lm(log(PRI) ~ geo + log(GAS) + DEP + log(CON) + INF + REN + log(GDP) + log(OIL) + MAR, D)
coef(LMc)[42:length(coef(LMc))]
```
...
tLM <- regsubsets(log(PRI) ~ poly(log(GAS),2) + poly(DEP,2) + poly(log(CON),2) + poly(INF,2) + poly(REN,2) + poly(log(GDP),2) + poly(log(OIL),2) + poly(MAR,2) + EU, DP[train,], nvmax = 18, weights=W)
