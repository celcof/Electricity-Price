...
library(leaps)
nVar <- 7
LM2 <- regsubsets(PRI ~ DEP + CON + GDP + OIL + REN + EMI + MAR, C, nvmax = nVar)
...
```{R}
tLM <- regsubsets(PRI ~ DEP + CON + GDP + OIL + REN + EMI + MAR, C, subset=train, nvmax = nVar)
test.mat <- model.matrix(PRI ~ DEP + CON + GDP + OIL + REN + EMI + MAR, data=C[test,])
...
```{r}
x <- model.matrix(PRI ~ DEP + CON + GDP + OIL + REN + EMI + MAR, C)[,-1]
y <- C$PRI
set.seed(35)
