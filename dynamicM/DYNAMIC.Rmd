...
lR <- glm(increase ~ DEP + CON + GDP + REN + EMI + MAR + EU, C, family=binomial, subset=train)
...
```{r}
library(tree)
val <- (C$time[train] >= 2012)
traini <- !(val)
tre <- tree(PRI ~ GAS + DEP + CON + EMI + MAR + REN + GDP + EU, C, subset=traini)
plot(tre)
text(tre,cex=1,digits=3)
```

From 2014 on -> test data to decide where to prune.

```{r}
my.tree = prune.tree(tre)
plot(my.tree)
```
