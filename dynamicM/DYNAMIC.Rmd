...
library(leaps)
nVar <- 8
LM2 <- regsubsets(PRI ~ DEP + CON + GDP + OIL + REN + EMI + MAR + INF, C, nvmax = nVar)
...
```{R}
tLM <- regsubsets(PRI ~ DEP + CON + GDP + OIL + REN + EMI + MAR + INF, C, subset=train, nvmax = nVar)
test.mat <- model.matrix(PRI ~ DEP + CON + GDP + OIL + REN + EMI + MAR + INF, data=C[test,])
val.errors <- rep(NA, nVar)
for (i in 1:nVar){
  coefi <- coef(tLM, id=i)
  pred <- test.mat[,names(coefi)] %*% coefi
  val.errors[i] <- mean(abs(C$PRI[test] - (C$PRI[test] + pred)))
}
...
```{r}
x <- model.matrix(PRI ~ DEP + CON + GDP + OIL + REN + EMI + MAR + INF, C)[,-1]
y <- C$PRI
set.seed(35)
