...
LM2 <- lm(PRI ~ DEP + CON + GDP + OIL + REN + CPI + NUC + EMI + MAR + geo + CRI, D)
gvlma::gvlma(LM2)
```

Better behaviour but still assumptions of linearity and of continuousness of my dependent variable are not met. Also, normality of errors is not met.
...
```{r}
plot(LM2, 1)
ggdensity(LM2$residuals, main = "Density function NUC", xlab = "Nuclear Capacity")
```
...
Finally, we also take the logarithm of the dependent variable, we manage to pass all tests.

```{r}
LM3 <- lm(log(PRI) ~ DEP + CON + GDP + OIL + REN + CPI + NUC + EMI + MAR + geo + CRI, D)
gvlma::gvlma(LM3)
```
...
```{R}
library(leaps)
LM3 <- regsubsets(log(PRI) ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, D, nvmax = 36)
...
```{R}
tLM <- regsubsets(log(PRI) ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, D, subset=train, nvmax = 36)
test.mat <- model.matrix(log(PRI) ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, data=D[test,])
...
```{r}
x <- model.matrix(log(PRI) ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, D)[,-1]
y <- D$PRI
set.seed(35)
...
As already explained, logarithmic transformation is used because lighten the weight on the model of some high leverage points. But let's see what happens with a lasso with no log transformation applied.
