...
Alternative. Take the log of PRI, CON, GDP, OIL and EMI and remove the two CY records and the variable NUC. We also normalize the value of NUC between 0 and 1.
...
LM2 <- lm(log(PRI) ~ DEP + log(CON) + log(GDP) + log(OIL) + REN + CPI + NUC + log(EMI) + MAR + geo + CRI, D)
gvlma::gvlma(LM2)
```
...
```{R}
library(leaps)
LM3 <- regsubsets(log(PRI) ~ DEP + log(CON) + log(GDP) + log(OIL) + REN + CPI + log(EMI) + MAR + geo + NUC + CRI, D, nvmax = 36)
which.max(summary(LM3)$adjr2)
which.min(summary(LM3)$bic)
which.min(summary(LM3)$cp)
coef(LM3,which.max(summary(LM3)$adjr2))
```

Optimal values for adjusted R squared and cp are obtained with 32 values while 28 variables are enough for bic. MAR, NUC and OIL are dropped. Rather than using proxies for test error, let us directly compute this error through validation set approach.
...
Simple linear regression on train now. To compare accuracies of models we are going to use Mean Absolute Error (for interpretability reasons). Since our dependent variable is measured on a logarithmic scale we have to have to take the exponential of the predictions vectors.

```{R}
tLM <- regsubsets(log(PRI) ~ DEP + log(CON) + log(GDP) + log(OIL) + REN + CPI + log(EMI) + MAR + geo + NUC + CRI, D, subset=train, nvmax = 35)
test.mat <- model.matrix(log(PRI) ~ DEP + log(CON) + log(GDP) + log(OIL) + REN + CPI + log(EMI) + MAR + geo + NUC + CRI, data=D[test,])
...
```{r}
x <- model.matrix(log(PRI) ~ DEP + log(CON) + log(GDP) + log(OIL) + REN + CPI + log(EMI) + MAR + geo + NUC + CRI, D)[,-1]
y <- D$PRI
set.seed(35)
...
What is the test average difference between the prediction and the actual log of the price of electricity?
...
```{r}
set.seed(35)
obj.escv <- escv.glmnet(x[train,], log(y[train]))
...
As already explained, logarithmic transformation is used because lighten the weight on the model of some high leverage points. But happened with a lasso with no log transformation applied.

```{r}
x <- model.matrix(PRI ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, D)[,-1]
...
```{r}
tLM <- regsubsets(PRI ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, D, subset=train, nvmax = 37)
test.mat <- model.matrix(PRI ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, data=D[test,])
...
A sign that maybe that maybe those log transformations could be not best decision.