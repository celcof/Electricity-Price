...
```{r}
LM3 <- lm(nthroot(PRI, 2) ~ DEP + CON + GDP + OIL + REN + CPI + NUC + EMI + MAR + geo + CRI, D)
gvlma::gvlma(LM3)
```
...
```{r}
ggdensity(sqrt(D$PRI), main = "Density function NUC", xlab = "Nuclear Capacity")
```
...
```{R}
library(leaps)
LM3 <- regsubsets(nthroot(PRI, 2) ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, D, nvmax = 36)
...
```{R}
tLM <- regsubsets(nthroot(PRI, 2) ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, D, subset=train, nvmax = 36)
test.mat <- model.matrix(nthroot(PRI, 2) ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, data=D[test,])
...
```{r}
x <- model.matrix(nthroot(PRI, 2) ~ DEP + CON + GDP + OIL + REN + CPI + EMI + MAR + geo + NUC + CRI, D)[,-1]
y <- D$PRI
set.seed(35)
ridge <- cv.glmnet(x[train,], nthroot(y[train], 2), alpha=0, standardize=T)
...
```{r}
lasso.mod <- glmnet(x[train,], nthroot(y[train], 2), alpha=1)
plot(lasso.mod)
```
...
```{r}
set.seed(35)
cv.out <- cv.glmnet(x[train,], nthroot(y[train], 2), alpha=1)
...
```{r}
set.seed(35)
obj.escv <- escv.glmnet(x[train,], nthroot(y[train], 2))
obj <- Lasso(x[train,], nthroot(y[train], 2), lambda = obj.escv$lambda.cv)
